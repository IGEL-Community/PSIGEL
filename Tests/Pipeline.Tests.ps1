...
Describe "$Script:FunctionName Integration Tests" -Tag "IntegrationTests" {
  ...
  Context "ParameterSetName UMSDevice" {

    $TestCfg = (($Cfg.Tests).where{ $_.Function -eq $FunctionName }).ParameterSets.UMSDevice
    ...
    It "doesn't throw" {
      ...
      { $Script:Result = @(
          $Params1 |
          New-UMSDevice #|
          #Move-UMSDevice -DestId $Params1.DeviceDirectoryId |
          #Update-UMSDevice -Name $Params1.DeviceUpdateName |
          #Remove-UMSDevice
        ) } | Should Not Throw
    }
